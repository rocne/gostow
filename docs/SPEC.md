...
```
gostow <semver> (GNU Stow 2.4.1 compatible)
```

The version is gostow's own — `gostow 0.1.1 (GNU Stow 2.4.1 compatible)` at the time of
writing. It is written as a placeholder here because a spec that pins a release number is a
spec that is wrong after the next one.
...
| PL-12 | `--version` prints `basename($0) (GNU Stow) version 2.4.1`, leaving no room for gostow's own build version. | source | — | **RULED (2026-07-09): diverge.** `--version` reports gostow's version, naming the conformant stow version alongside: `gostow <semver> (GNU Stow 2.4.1 compatible)`. Parity is owed to behaviour scripts depend on, not to self-identification. The only intentional output divergence. §4.4 |
...
| PL-24 | **Neither of GNU Stow's option references is complete, and they omit different options.** `stow --help` never names `--no-folding` (that is PL-16, seen from the other side); `stow.8` never names `--compat` or its short form `-p`. A user reading either one alone concludes an option does not exist. | **probed (2026-07-10): confirmed** | 1 | **RULED: document the union.** Both omissions are documentation bugs, not behaviour — the options parse and work in both stow and gostow, and the differential suite pins that. `man/gostow.8` documents every option `spec()` accepts, and two tests hold it there: `internal/cli/docs_test.go` requires the page and the three completion scripts to name *exactly* `spec()`'s options, and `TestManPageDocumentsEveryOptionStowDocumentsAnywhere` requires gostow's page to cover `stow --help` ∪ `stow.8` — asserting, as its own premise, that the two really do disagree. Found 2026-07-10 while writing gostow's man page. Report upstream, with PL-01 (also a doc bug). |

**Upstream bug reports to file:** PL-01, PL-03, PL-04, PL-05, PL-06, PL-08, PL-09, PL-10, **PL-18** (highest severity: a `$HOME` containing a regex metacharacter makes stow unusable), PL-21, PL-24.
